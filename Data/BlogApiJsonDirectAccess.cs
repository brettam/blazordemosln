...
public class BlogApiJsonDirectAccess : IBlogApi
{
    ...
    BlogApiJsonDirectAccess(IOptions<BlogApiJsonDirectAccessSetting> options)
    {
    ...
    private async Task SaveAsync<T>(List<T>? list, string folder, string filename, T item)
    {
        ...
        var fullpath = $@"{_settings.DataPath}\{folder}\{filename}";
        await File.WriteAllTextAsync(filename, JsonSerializer.Serialize<T>(item));
        if (list == null)
        {
            list = new();
        }
